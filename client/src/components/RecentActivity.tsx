...
export function RecentActivity({ notes }: RecentActivityProps) {
  const topNotes = notes.filter(n => n.type === 'text' || n.type === 'code').slice(0, 3)
  const imageNote = notes.find(n => n.type === 'image')
  const checklistNote = notes.find(n => n.type === 'checklist')
