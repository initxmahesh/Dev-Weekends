  ...
  return (
    <section className="mb-8">
      <div className="flex items-center gap-2 mb-4">
        <Pin size={12} className="text-[#636a8a]" />
        ...
        {pinnedNotes.map(note => (
          ...
          >
            <div className="flex items-center justify-between gap-3">
              ...
              <span className="text-[11px] text-[#636a8a]">Last edited: {note.lastEdited}</span>
            </div>

            <h3 className="text-[14.5px] font-semibold text-[#eef0fb] leading-snug tracking-tight">
              {note.title}
            </h3>

            <p className="text-[12.5px] text-[#8b91b8] leading-relaxed line-clamp-3 flex-1">
              {note.description}
            </p>
            ...
            <div className="flex flex-wrap gap-1.5">
              {note.tags.map(tag => (
                <span
                  key={tag}
                  className="text-[11px] text-[#636a8a] bg-white/[0.03] border border-white/[0.05] rounded px-2 py-0.5"
                >
                  {tag}
                </span>
              ))}
